...
The repo supports pnpm, bun, and yarn. Use mise tasks to switch between them:
...
```sh
mise run pnpm:build    # or bun:build or yarn:build
mise run pnpm:test     # or bun:test or yarn:test
mise run pnpm:typecheck # or bun:typecheck or yarn:typecheck
```
...
- What’s the urgency of adding support for other package managers, like regular npm?
