...
Dependicus supports pnpm, bun, yarn, and mise as dependency providers. For npm-based package managers it reads the lockfile, workspace structure, and catalog configuration. For mise it reads `mise.toml` and queries the mise CLI for tool versions.
...
pnpm and bun support dependency catalogs, which centralize version ranges so workspace packages can reference them instead of duplicating version specifiers. yarn and mise do not have a native catalog feature, so `isCatalog` always returns false for those providers.
...
The mise provider differs from the npm-based providers in a few ways:
...
| Capability         | pnpm                            | bun                        | yarn                                   | mise             |
| ------------------ | ------------------------------- | -------------------------- | -------------------------------------- | ---------------- |
| Dependency listing | `pnpm -r list --json --depth=0` | Parses `bun.lock` directly | Parses `yarn.lock` directly            | `mise ls --json` |
| Catalog            | `pnpm-workspace.yaml`           | `package.json`             | Not supported                          | Not supported    |
| Patched packages   | Yes                             | No                         | Yes (`patch:` protocol in `yarn.lock`) | No               |
| Lockfile           | `pnpm-lock.yaml`                | `bun.lock`                 | `yarn.lock`                            | `mise.toml`      |
| Publish dates      | Yes (npm registry)              | Yes (npm registry)         | Yes (npm registry)                     | No               |
| Ecosystem          | npm                             | npm                        | npm                                    | mise             |

## CI considerations

In CI, the runtime detection step relies on the process being launched by pnpm, bun, or yarn. Mise has no runtime detection and is always detected via `mise.toml` presence. If you invoke your Dependicus script with bare `node`, auto-detection falls back to lockfile presence. You can make detection explicit with `--provider`:
