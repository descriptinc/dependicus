...
import type { FactStore, ProviderOutput, DependencyProvider } from './core/index';
import { detectNodeProviders, createNodeProvidersByName } from './providers-node/index';
import { MiseProvider } from './provider-mise/index';
import { UvProvider } from './providers-python/index';
...
function detectProviders(cacheService: CacheService, repoRoot: string): DependencyProvider[] {
    ...
    const uvProvider = new UvProvider(cacheService, repoRoot);
    ...
    const cargoProvider = new CargoProvider(cacheService, repoRoot);
    if (cargoProvider.discoverProjectDirs().length > 0) {
        providers.push(cargoProvider);
    }
    return providers;
}
...
function allProviders(cacheService: CacheService, repoRoot: string): DependencyProvider[] {
    return [
        ...
        new CargoProvider(cacheService, repoRoot),
    ];
}
    ...
    return {
        async run(argv: string[]): Promise<void> {
            ...
            program
                ...
                .option('--output-dir <path>', 'Directory to write HTML and JSON output')
                .option('--cache-dir <path>', 'Directory to store cached API data')
                .option('--site-name <name>', 'Name shown in site heading and title');
            ...
            function resolveConfig() {
                ...
                const repoRoot = resolve(globalOpts.repoRoot ?? config.repoRoot ?? '.');
                const providerNames =
                    globalOpts.provider.length > 0 ? globalOpts.provider : config.providerNames;
            ...
            program
                ...
                .option(
                    '--cooldown-days <days>',
                    'Days to wait before creating issues for new versions',
                )
                .option('--rate-limit-days <days>', 'Default notification rate limit in days')
                .option('--no-new-issues', 'Prevent creation of new issues (only update existing)')
                .option(
                    '--skip-state <name>',
                    'Skip updating issues in this Linear state (repeatable)',
                    collect,
                    [] as string[],
                )
                .action(
                            ...
                            store,
                            {
                                ...
                                cooldownDays:
                                    options.cooldownDays != null
                                        ...
                                        : linearConfig.cooldownDays,
                                allowNewIssues: !options.newIssues
                                    ? false
                                    : linearConfig.allowNewIssues,
                                skipStateNames:
                                    options.skipState.length > 0
                                        ? options.skipState
                                        : linearConfig.skipStateNames,
                                rateLimitDays:
                        ...
                        await reconcileGitHubIssues(
                            ...
                            store,
                            {
                                ...
                                cooldownDays:
                                    options.cooldownDays != null
                                        ? Number(options.cooldownDays)
                                        : githubConfig.cooldownDays,
                                allowNewIssues: !options.newIssues
                                    ? false
                                    : githubConfig.allowNewIssues,
                                rateLimitDays:
                                    options.rateLimitDays != null
                                        ? Number(options.rateLimitDays)
                                        : githubConfig.rateLimitDays,
                            },
