...
describe('MiseProvider', () => {
    ...
    it('parses mise ls output into per-config-file packages', async () => {
        ...
        vi.mocked(execSync).mockReturnValueOnce(gitLsOutput); // git ls-files
        vi.mocked(execFileSync).mockReturnValueOnce(miseOutput); // mise ls --json
    ...
    it('filters out tools inherited from parent configs', async () => {
        ...
        vi.mocked(execSync).mockReturnValueOnce(gitLsOutput);
        vi.mocked(execFileSync)
    ...
    it('resolves latest versions from mise outdated across directories', async () => {
        ...
        // git ls-files uses execSync
        vi.mocked(execSync).mockReturnValueOnce(gitLsOutput);
        // mise outdated and mise ls use execFileSync
        vi.mocked(execFileSync)
            .mockReturnValueOnce(rootOutdatedOutput) // mise outdated for root
            .mockReturnValueOnce(docsOutdatedOutput) // mise outdated for docs
            .mockReturnValueOnce(rootMiseOutput) // mise ls for root
            .mockReturnValueOnce(docsMiseOutput); // mise ls for docs
