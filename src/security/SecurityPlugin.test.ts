...
describe('SecurityPlugin', () => {
    ...
    it('handles null cvssScore without crashing', () => {
        const plugin = new SecurityPlugin({ githubAdvisory: true });
        const store = new RootFactStore();
        const scoped = store.scoped('npm');
        const ver = makeVersion();

        // GitHub API can return null for cvss.score; simulate a finding where
        // cvssScore leaked through as null instead of undefined.
        // Use JSON.parse to inject null without a null literal.
        const finding = JSON.parse(
            JSON.stringify({
                ...
                rationale: ['1 GitHub advisory (GHSA-0000)'],
                sourceLinks: [
                    { label: 'GHSA-0000', url: 'https://github.com/advisories/GHSA-0000' },
                ],
            }),
        ) as SecurityFinding;
        // Inject null cvssScore as the GitHub API would
        (finding as unknown as Record<string, unknown>).cvssScore = JSON.parse('null');
        (finding.advisories![0] as unknown as Record<string, unknown>).cvssScore =
            JSON.parse('null');

        scoped.setVersionFact('some-pkg', '1.0.0', SECURITY_FINDINGS_KEY, [finding]);

        const ctx = { name: 'some-pkg', version: ver, store: scoped, ecosystem: 'npm' };

        // Column rendering should not throw
        for (const col of plugin.columns) {
            expect(() => col.getValue(ctx)).not.toThrow();
        }
        // Tooltip should not throw
        const secCol = plugin.columns.find((c) => c.key === 'security')!;
        expect(() => secCol.getTooltip!(ctx)).not.toThrow();

        // Linear issue spec should not throw
        const spec = plugin.getLinearIssueSpec(
            {
                name: 'some-pkg',
                ecosystem: 'npm',
